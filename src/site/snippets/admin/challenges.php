...
<ul class="nav nav-tabs" id="adminTab" role="tablist">
  <li class="nav-item" role="presentation">
    <button class="nav-link<?=($selectedTab==='add') ? ' active' : '' ?>" id="add-tab" data-bs-toggle="tab" data-bs-target="#add" type="button" role="tab"
      aria-controls="add"
      aria-selected="<?=($selectedTab==='add') ? 'true' : 'false' ?>"><?= t("Adding a New Challenge for", "Adding a New Challenge for") . ' '.  $team['country_name'] ?></button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link<?=($selectedTab==='edit') ? ' active' : '' ?>" id="edit-tab" data-bs-toggle="tab" data-bs-target="#edit" type="button" role="tab"
      aria-controls="edit"
      aria-selected="<?=($selectedTab==='edit') ? 'true' : 'false' ?>"><?= t("Edit an existing Challenge", "Edit an existing Challenge") ?></button>
  </li>
</ul>
      ...
      <td style="width:75%">
        <form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
          ...
          <div class="mb-3">
            ...
            <button class="btn btn-primary btn-sm" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $challenge['id'] ?>" aria-expanded="false" aria-controls="collapse<?= $challenge['id'] ?>">
              <?= t("EDIT", "EDIT") ?>
            </button>
          </div>
          <div class="collapse" id="collapse<?= $challenge['id'] ?>">
            ...
            <button class="btn btn-primary btn-sm" type="button" data-bs-toggle="collapse" data-bs-target="#topicCollapse<?= $challenge['id'] ?>" aria-expanded="false" aria-controls="topicCollapse<?= $challenge['id'] ?>">
...
?>
            ...
            <input type="submit" class="btn btn-primary" value="<?= t("UPDATE CHALLENGE","UPDATE CHALLENGE") ?>">
            </form>
          </div>
      </td>
      <td>
